...
# ======================================
# LOAD FACENET – THE BEST FOR MATCHING
# ======================================
facenet = hub.KerasLayer(
    "https://tfhub.dev/tensorflow/facenet/512/1",
...
# -----------------------------------------------------------
# L2 NORMALIZATION (Required for FaceNet)
# -----------------------------------------------------------
def l2_normalize(v):
    return v / np.linalg.norm(v, axis=1, keepdims=True)
...
async def compare_images(payload: dict):
    try:
        ...
        emb1 = facenet(img1).numpy()
        emb2 = facenet(img2).numpy()
        ...
        # Euclidean Distance (FaceNet standard threshold)
        euclidean_dist = float(np.linalg.norm(emb1 - emb2))

        # FaceNet recommended threshold
        match = euclidean_dist < 1.0  
